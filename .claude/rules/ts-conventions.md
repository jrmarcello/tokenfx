...
- Use `@tokenfx/shared/logger` with `debug` / `info` / `warn` / `error`. The logger emits in tests unless `LOG_LEVEL` suppresses the level; tests that capture logs stub `log.warn`/`log.error` by property mutation (hand-written stub).
