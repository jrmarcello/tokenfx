...
| TC-I-45 | REQ-18 | security | **ADDRESSED** by `.specs/sso-nonce-replay.md` (operationally equivalent — Auth.js v5 runs state-check before nonce-check, so a literal URL-replay fires the state branch first; the new spec instead pins a tampered `id_token.nonce` via the stub and drives a single callback with valid state, isolating the nonce-validation path). Original: Replay-detection via nonce reuse: second callback with fresh state + reused nonce → rejected with NextAuth nonce error + `'rejected-replay'` audit row | match |
